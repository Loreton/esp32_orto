...
void beep(int count)
{
    LOG_DEBUG("ACTIVE_BUZZER_pin beep x%d", count);
...
void openValve()
{
    if (valveOpen) {
        ...
        LOG_INFO("Apertura richiesta ma valvola già aperta");
        return;
    }
...
void beep(int n) {
  for(int i=0;i<n;i++) {
    digitalWrite(ACTIVEACTIVE_BUZZER_PIN_pin, HIGH);
    delay(120);
    digitalWrite(ACTIVEACTIVE_BUZZER_PIN_pin, LOW);
    delay(120);
  }
}
...
void setup() {
  ...
  pinMode(EV_RELAY_OPEN_pin, OUTPUT);
  pinMode(EV_RELAY_CLOSE_pin, OUTPUT);
  pinMode(ACTIVEACTIVE_BUZZER_PIN_pin, OUTPUT);
